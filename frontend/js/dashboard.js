...
async function loadActiveRounds() {
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            if(heroRoundBadge) heroRoundBadge.textContent = `STANDBY`;
            if(heroRoundTitle) heroRoundTitle.innerHTML = `WAITING FOR<br /><span class="text-blood" style="animation: flicker 4s infinite">NEXT ROUND</span>`;
        }
    } catch (error) {
